...
export function Card(props) {
  ...
  const [cardId, setCardId] = useState(
    props.card["card_id"] ? props.card["card_id"] : props.card["id"]
  );
  const [cardAmount, setCardAmount] = useState(
    props.card["card_count"] ? props.card["card_count"] : 1
  );
  ...
  const updateCard = (cardAmount) => {
    ...
    apiUserCardsUpdate(handleBackendUpdate, props.card["id"], {
    ...
    editCard = (
      <React.Fragment>
        <div className={Classes.userQty}>
          <div onClick={changeInput} className={Classes.userMinus}>
            -
          </div>
          <div className={Classes.userCount}>{cardAmount}</div>
          <div onClick={changeInput} className={Classes.userPlus}>
            +
          </div>
        </div>
        <div className={Classes.buttonWrapper}>
          ...
          {message && <span className={Classes.cardAdded}>{message}</span>}
        </div>
      </React.Fragment>
    );
  } else {
    editCard = (
      <React.Fragment>
        <div className={Classes.qty}>
          <div onClick={changeInput} className={Classes.minus}>
            -
          </div>
          <div className={Classes.count}>{cardAmount}</div>
          <div onClick={changeInput} className={Classes.plus}>
            +
          </div>
        </div>
        <div className={Classes.buttonWrapper}>
  ...
  return (
    <React.Fragment>
      <div className={Classes.cardWrapper}>
        <img src={`${props.card["img_url"]}.jpg`} alt={props.card["name"]} />
        <div className={Classes.editCard}>
          <a href={`/card_detail/${cardId}`}>{props.card["name"]}</a>
          {editCard}
        </div>
      </div>
    </React.Fragment>
...
function CardList(props) {
  ...
  const changeColorDisplayedHandler = (event, color, index) => {
    ...
    setColor({ color: color, cards: index });
  };
  useEffect(() => {
  ...
  return (
    <React.Fragment>
      ...
      <h2 className={Classes.colorTitle}>
      ...
      {cards.map((list, index) => {
        return (
          <div
            key={index}
            className={
              index === color["cards"] ? Classes.cardsContainer : Classes.hidden
            }
